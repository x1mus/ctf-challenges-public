...
with open('check.py', 'r') as f:
	...
	root = ast.parse(code)
	scrambled = Scrambler(True).visit(root)
	with open('out.py', 'w') as of:
		of.write(Scrambler.HEADER)
		of.write(decompile(scrambled))

with open("out.py", 'r') as f:
	f.readline()
	code = ''.join(f.readlines())
	root = ast.parse(code)
	unscrambled = Scrambler(False).visit(root)
	with open("unscrambled.py", "w") as of:
		of.write(decompile(unscrambled))